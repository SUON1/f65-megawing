...
|---|---|---|
| Safe FCM path | `r0b_vic4_fcm_safe_gate()` checks the candidate ledger and performs no VIC I/O; target disassembly is rejected if it contains `$D054`. | `R0B-FCM-SAFE-001 DEFERRED` — clear only with an isolated physical text-restore proof for a state-changing FCM register operation. |
...
| Hardware run | The target screen asks for owner capture; the guide requires the matching result block and machine identity. | `R0B-HW-001 OWNER CAPTURE REQUIRED` until returned owner evidence. |
