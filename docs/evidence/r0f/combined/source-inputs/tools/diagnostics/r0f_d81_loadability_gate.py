#!/usr/bin/env python3
"""Fail-closed structural and extracted-content gate for the assigned R0-F carrier."""
import hashlib, json, os, pathlib, subprocess, sys, tempfile
if os.environ.get('F65_R0F_VARIANT') == 'combined':
 from r0f_combined_build import OUT, CARRIER_NAME, CARRIER_LABEL
else:
 from r0f_build import OUT, CARRIER_NAME, CARRIER_LABEL

root=pathlib.Path(sys.argv[1]).resolve(); image=pathlib.Path(sys.argv[2]).resolve(); art=OUT; out=OUT
def fail(s): raise SystemExit("R0-F D81 loadability gate failed: "+s)
def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
if image != OUT/CARRIER_NAME: fail("unexpected or retired carrier identity: "+image.name)
...
h=sec(40,0)
if h[2] != 0x44: fail("header DOS version")
for sector, link in [(1, bytes((40,2))), (2, bytes((0,255)))]:
 bam=sec(40,sector)
 if bam[:2]!=link or bam[2:4]!=bytes((0x44,0xbb)) or bam[4:6]!=h[22:24]: fail("BAM header/link/ID")
...
if (label,ident)!=(CARRIER_LABEL,"65"): fail("disk label/ID mismatch")
free=set()
for t in range(1,81):
...
while cur:
 ...
 if cur in {(40,0),(40,1),(40,2)} or cur[0]!=40: fail("directory metadata overlap/range")
 dirs.add(cur); z=sec(*cur);
 if z[0]==0 and z[1]!=255: fail("directory terminator")
 cur=(z[0],z[1]) if z[0] else None
...
expected=[("autoboot.c65",art/"AUTOBOOT.C65"),("r0f-proof",art/"F65-R0F-PROOF.prg"),("r0f-evid",art/"R0F-EVID.txt")]
...
if not reported or int(reported.group(1)) != sum(t!=40 for t,s in free): fail("listing free-block accounting")
for n,p in expected:
 if payload[n]!=p.read_bytes(): fail("raw payload mismatch "+n)
 with tempfile.TemporaryDirectory(prefix="r0f-d81-") as td:
...
release={"D81_STATE":"HOST_CONTENT_VERIFIED","D81_FILENAME":image.name,"D81_SHA256":sha(image),"D81_BYTES":819200,"DISK_LABEL":label,"DISK_ID":ident,"ENTRY_FILENAME":"AUTOBOOT.C65 -> R0F-PROOF","SOURCE_BRANCH":branch,"SOURCE_COMMIT":commit,"SOURCE_STATE":"working tree; exact input hashes in R0F-EVID.txt","BUILDER_IDENTITY":{"path":str(c1541),"sha256":sha(c1541),"version":b["version"],"realdevice":"disabled"},"STRUCTURAL_VALIDATOR_IDENTITY":{"path":"tools/diagnostics/r0f_d81_loadability_gate.py","sha256":sha(pathlib.Path(__file__))},"HOST_STRUCTURAL_RESULT":"PASS","HOST_CONTENT_RESULT":"PASS","XEMU_RESULT":"AWAITING_STAGE_3","XEMU_EVIDENCE":None,"SD_COPY_SHA256":None,"SD_FILESYSTEM":None,"SD_TRANSFER_METHOD":None,"SD_EXTENT_COUNT":None,"SD_EXTENT_EVIDENCE":None,"SD_SAFE_EJECT_RESULT":"NOT VERIFIED","SD_CONTIGUITY_RESULT":"AWAITING_AUTHORIZED_TRANSFER","PHYSICAL_CHOOSER_RESULT":"AWAITING_HUMAN","PHYSICAL_EVIDENCE":None,"payloads":[{"hostFilename":p.name,"petsciiFilename":n,"bytes":p.stat().st_size,"sha256":sha(p)} for n,p in expected],"construction":"fresh format plus all payload writes in one pinned c1541 invocation","c1541Listing":listing}
(out/"manifests").mkdir(parents=True,exist_ok=True); (out/"reports").mkdir(parents=True,exist_ok=True)
(out/"manifests/r0f-d81-release.json").write_text(json.dumps(release,indent=2,sort_keys=True)+"\n")
(out/"reports/R0F-D81-LOADABILITY.md").write_text("# R0-F D81 Loadability\n\nState: `HOST_CONTENT_VERIFIED`\n\n- Candidate: `%s`\n- SHA-256: `%s`\n- Host structural/content: `PASS`\n- Xemu: `AWAITING_STAGE_3`\n- SD transfer: `AWAITING_AUTHORIZED_TRANSFER`\n- Physical chooser: `AWAITING_HUMAN`\n"%(image.name,sha(image)))
print("R0-F D81 STRUCTURAL+CONTENT PASS sha256="+sha(image))
