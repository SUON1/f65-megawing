...
`F65R0D.D81` remains invalid after physical chooser FF and is not an input to
this carrier. `F65R0D2.D81` is not final or `TEST_ELIGIBLE` until a copied-file
hash and physical chooser PASS are recorded.
