...
30 d=9
40 trap 9000:row=0:for k=0 to 1999:poke 2048+k,32:next k:t$="R0-C TWO-GENERATION MEDIA FIXTURE":gosub 9600
50 t$="SACRIFICIAL WRITABLE MEDIA ON DEVICE 9 ONLY":gosub 9600
60 t$="DEVICE 8 LOADED THIS FIXTURE; DEVICE 9 ONLY IS PROBED.":gosub 9600
...
1010 t$="INITIALIZE WRITES ONLY R0C FIXTURE FILES ON DEVICE 9.":gosub 9600
1020 t$="PRESS Y TO CONTINUE":gosub 9600:gosub 9700:if a<>89 then return
1030 rem blank media is valid; @0 writes below replace only R0C fixture files.
...
2010 gosub 5100:if aa<0 then t$="NO VALID SELECTOR - INITIALIZE OR RECOVER FIRST":gosub 9600:return
2020 ns=1-aa:ng=bb+1:t$="WRITING CANDIDATE GENERATION"+str$(ng)+" TO SLOT"+str$(ns):gosub 9600
...
4510 ok=0:open 2,d,2,"@0:R0CSEL,S,W"
4520 print#2,"R0CSEL";",";ns;",";ng;",";"COMMIT":close 2
4530 gosub 5100
4540 if aa<>ns then return
4550 if bb<>ng then return
4560 ok=1:t$="SELECTOR PASS: SLOT"+str$(aa)+" GENERATION"+str$(bb):gosub 9600:return
5000 rem recover from selector plus the two independently verified generations.
5010 gosub 5200:gosub 5300:gosub 5100
5020 if aa=0 and v0=bb then t$="RECOVERY PASS: SELECTOR CHOSE G0"+str$(bb):gosub 9600:return
5030 if aa=1 and v1=bb then t$="RECOVERY PASS: SELECTOR CHOSE G1"+str$(bb):gosub 9600:return
...
5110 aa=-1:bb=-1:m$="":c$=""
5120 open 2,d,2,"R0CSEL,S,R"
5130 input#2,m$,aa,bb,c$
5140 close 2
5150 if m$<>"R0CSEL" then aa=-1:bb=-1:return
5160 if c$<>"COMMIT" then aa=-1:bb=-1:return
5170 if aa<0 or aa>1 then aa=-1:bb=-1
5180 return
5200 rem read and validate generation 0.
5210 v0=-1:open 2,d,2,"R0CG0,S,R":input#2,m$,g0,s0,p$
5220 if m$<>"R0CGEN" then close 2:return
5230 if s0<>0 then close 2:return
5240 if p$<>"R0C-FIXTURE" then close 2:return
5250 for n=1 to 512:input#2,x$,xn:if x$<>"R0C-PAD" then close 2:return
5252 if xn<>n then close 2:return
5254 next n
5256 input#2,x$,c$:close 2
5258 if x$<>"R0C-END" then return
5260 if c$<>"OK" then return
5262 v0=g0:return
5300 rem read and validate generation 1.
5310 v1=-1:open 2,d,2,"R0CG1,S,R":input#2,m$,g1,s1,p$
5320 if m$<>"R0CGEN" then close 2:return
5330 if s1<>1 then close 2:return
5340 if p$<>"R0C-FIXTURE" then close 2:return
5350 for n=1 to 512:input#2,x$,xn:if x$<>"R0C-PAD" then close 2:return
5352 if xn<>n then close 2:return
5354 next n
5356 input#2,x$,c$:close 2
5358 if x$<>"R0C-END" then return
5360 if c$<>"OK" then return
5362 v1=g1:return
...
6060 open 2,d,2,"@0:"+f$+",S,W":print#2,"R0C-CORRUPT":close 2
...
8010 gosub 5100:if aa<0 then t$="NO VALID SELECTOR - INITIALIZE FIRST":gosub 9600:return
8020 ns=1-aa:ng=bb+1:gosub 3000:gosub 4000:if ok=0 then return
...
9030 goto 40
9500 rem legacy placeholder: BASIC error trap is authoritative.
9510 return
9600 col=0
