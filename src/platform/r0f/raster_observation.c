...
/*
 * Private R0-F diagnostic helper.  It reads only the VIC-II-compatible
 ...
 */
uint8_t r0f_raster_low_read(void) { return VICII.rasterline; }

uint8_t r0f_raster_wait_low_phase(uint8_t phase) {
  uint16_t attempts;
  for (attempts = 0u; attempts != 0xffffu; ++attempts)
    if (r0f_raster_low_read() == phase) return 1u;
  return 0u;
}
