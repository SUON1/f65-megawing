...
extern uint8_t r0b_d031_restore_probe(void);
extern uint8_t r0b_d031_saved;
extern uint8_t r0b_d031_target;
extern uint8_t r0b_d031_readback;
...
static void initialise_result(void) {
  ...
  result->header.environment = 0u;
  result->header.status = DEFERRED;
  result->header.harness_revision = 5u;
...
void r0b_d031_safe_run(void) {
  ...
  initialise_result();
  clear_screen();
  text(0u, "R0-B ISOLATED D031 40-COL RESTORE");
  text(1u, "NO D02F; NO D054; NO PTR; NO DMA; NO MAP");
  text(2u, "TEXT RESTORE SENTINEL");
  text(3u, "PHYSICAL EVIDENCE: PENDING");

  flags = r0b_d031_restore_probe();
  text_ok = sentinel_unchanged();
  pass = (uint8_t)((flags & 0x07u) == 0x07u && text_ok != 0u);

  text(5u, pass ? "R0B-D031-SAFE-001 LOCAL TEST: PASS" : "R0B-D031-SAFE-001 DEFERRED/FAIL");
  text(6u, (flags & 0x01u) ? "C65 CONTEXT: PASS" : "C65 CONTEXT: NOT OBSERVED");
  text(7u, (flags & 0x02u) ? "D031 40-COL READBACK: PASS" : "D031 40-COL READBACK: FAIL");
  text(8u, (flags & 0x04u) ? "D031 EXACT RESTORE: PASS" : "D031 EXACT RESTORE: FAIL");
  ...
  text(24u, "GATE OPEN; FCM CARD/SWAP NOT ENABLED");
  ...
  result->reserved[2] = r0b_d031_saved;
  result->reserved[3] = r0b_d031_target;
  result->reserved[4] = r0b_d031_readback;
