...
#define R0F_SNAPSHOT_COUNT 3u
#define R0F_CASE_COUNT 5u
#define R0F_STATE_FREE 0u
#define R0F_STATE_PUBLISHING 1u
#define R0F_STATE_READY 2u
#define R0F_STATE_READING 3u
#define R0F_CASE_LAG 1u
#define R0F_CASE_SHEDDING 2u
#define R0F_CASE_ONE_OVER 4u
#define R0F_CASE_INPUT_AUDIO_PRESSURE 8u
#define R0F_TIMING_UNIT_RASTER_LOW_BYTE_MODULO_256 1u
...
static uint8_t snapshot_records[R0F_SNAPSHOT_COUNT][R0F_SNAPSHOT_BYTES];


extern uint8_t r0f_raster_low_read(void);
extern uint8_t r0f_raster_wait_low_phase(uint8_t phase);
...
static uint8_t petscii(char c) { return (c >= 'A' && c <= 'Z') ? (uint8_t)((uint8_t)c & 0x1fu) : (uint8_t)c; }
...
static void reset_snapshots(void) {
  ...
  for (slot = 0u; slot != R0F_SNAPSHOT_COUNT; ++slot)
    for (byte = 0u; byte != R0F_SNAPSHOT_BYTES; ++byte) snapshot_records[slot][byte] = 0u;
}
static uint8_t find_state(uint8_t state) {
  uint8_t slot;
  for (slot = 0u; slot != R0F_SNAPSHOT_COUNT; ++slot)
    if (snapshot_records[slot][0] == state) return slot;
  return R0F_SNAPSHOT_COUNT;
}
/* Simulation alone owns FREE -> PUBLISHING -> READY. */
static void publish_snapshot(uint16_t tick, uint32_t checksum, proof_metrics *m) {
  uint8_t slot = find_state(R0F_STATE_FREE);
  if (slot == R0F_SNAPSHOT_COUNT) { ++m->skipped; return; }
  snapshot_records[slot][0] = R0F_STATE_PUBLISHING;
  ...
  snapshot_records[slot][0] = R0F_STATE_READY;
  ++m->published;
}
/* Presentation alone owns READY -> READING -> FREE. */
static void present_snapshot(void) {
  uint8_t slot = find_state(R0F_STATE_READY);
  if (slot == R0F_SNAPSHOT_COUNT) return;
  snapshot_records[slot][0] = R0F_STATE_READING;
  (void)snapshot_records[slot][3];
  snapshot_records[slot][0] = R0F_STATE_FREE;
}
...
static void proof_tick(proof_metrics *m, uint8_t flags, uint16_t tick) {
  uint8_t stage;
  m->ticks = tick;
  for (stage = 1u; stage <= R0F_STAGE_COUNT; ++stage) {
    if (stage == 2u) m->input_edges = (uint16_t)(m->input_edges + ((flags & R0F_CASE_INPUT_AUDIO_PRESSURE) ? 2u : 1u));
    if (stage == 19u) m->audio_services = (uint16_t)(m->audio_services + ((flags & R0F_CASE_INPUT_AUDIO_PRESSURE) ? 2u : 1u));
    if (stage == 20u) m->checksum = mix(m->checksum, tick);
  }
  publish_snapshot(tick, m->checksum, m);
  if (!(flags & R0F_CASE_LAG) || (tick % 5u) == 0u) present_snapshot();
...
static proof_metrics proof_case(uint8_t flags) {
  ...
  while (find_state(R0F_STATE_READY) != R0F_SNAPSHOT_COUNT) present_snapshot();
  return m;
}

/* Preserve phase order. Validity is separate: zero is a legitimate modulo byte.
 * Reset before phase acquisition so reset work does not displace the sample. */
static uint8_t observe_timing_case(uint8_t id, uint8_t flags) {
  uint8_t sample;
  uint16_t valid = 0u;
  for (sample = 0u; sample < R0F_TIMING_SAMPLE_COUNT; ++sample) {
    proof_metrics m = proof_begin();
    uint16_t tick;
    uint8_t start, found;
    reset_snapshots();
    found = r0f_raster_wait_low_phase((uint8_t)(sample << 4u));
    start = r0f_raster_low_read();
    for (tick = 1u; tick <= R0F_TIMING_WINDOW_TICKS; ++tick)
      proof_tick(&m, flags, tick);
    while (find_state(R0F_STATE_READY) != R0F_SNAPSHOT_COUNT) present_snapshot();
    result[(uint8_t)(R0F_TIMING_CASE_OFFSET + id * 16u + sample)] =
      (uint8_t)(r0f_raster_low_read() - start);
    if (found) valid |= (uint16_t)(1u << sample);
  }
  put16((uint8_t)(R0F_VALID_MASK_OFFSET + id * 2u), valid);
  return valid == 0xffffu;
}
...
void r0f_run(void) {
  ...
  for (i = 0u; i != R0F_RESULT_BYTES; ++i) result[i] = 0u;
  result[0] = 'R'; result[1] = '0'; result[2] = 'F'; result[3] = '1'; result[4] = 1u; result[5] = R0F_SIMULATION_HZ; result[6] = R0F_STAGE_COUNT;
  normal = proof_case(0u);
  lag = proof_case(R0F_CASE_LAG);
  shedding = proof_case(R0F_CASE_SHEDDING);
  one_over = proof_case(R0F_CASE_ONE_OVER);
  pressure = proof_case(R0F_CASE_INPUT_AUDIO_PRESSURE);
  normal_ok = (normal.ticks == 1000u && normal.published == 1000u && normal.skipped == 0u && normal.checksum == 393387319ul && normal.input_edges == 1000u && normal.audio_services == 1000u);
  lag_ok = (lag.ticks == 1000u && lag.skipped == 798u && lag.published == 202u && lag.checksum == normal.checksum);
  ...
  result[28] = R0F_SNAPSHOT_COUNT; result[29] = R0F_SNAPSHOT_BYTES; result[30] = 1u; result[31] = 0u; result[32] = R0F_CASE_COUNT;
  ...
  result[R0F_TIMING_RESULT_OFFSET] = 'R'; result[R0F_TIMING_RESULT_OFFSET + 1u] = 'T'; result[R0F_TIMING_RESULT_OFFSET + 2u] = 1u;
  result[R0F_TIMING_RESULT_OFFSET + 3u] = R0F_TIMING_SAMPLE_COUNT;
  result[R0F_TIMING_RESULT_OFFSET + 4u] = R0F_TIMING_WINDOW_TICKS;
  result[R0F_TIMING_RESULT_OFFSET + 5u] = R0F_CASE_COUNT;
  result[R0F_TIMING_RESULT_OFFSET + 6u] = R0F_TIMING_CASE_BYTES;
  result[R0F_TIMING_RESULT_OFFSET + 8u] = R0F_TIMING_UNIT_RASTER_LOW_BYTE_MODULO_256;
  timing_ok = (uint8_t)(observe_timing_case(0u, 0u) & observe_timing_case(1u, R0F_CASE_LAG) & observe_timing_case(2u, R0F_CASE_SHEDDING) & observe_timing_case(3u, R0F_CASE_ONE_OVER) & observe_timing_case(4u, R0F_CASE_INPUT_AUDIO_PRESSURE));
  result[R0F_TIMING_RESULT_OFFSET + 7u] = timing_ok ? 0x7fu : 0u;
  for (i = 0u; i != (R0F_RESULT_BYTES - 1u); ++i) sum = (uint8_t)(sum + result[i]);
  result[R0F_RESULT_BYTES - 1u] = sum;
  clear();
  line(0u, "R0-F COMBINED-LOAD FUNCTIONAL PROXY");
  line(2u, result[7] == 0x7fu ? "UNPACED 100HZ / 21-STAGE MODEL: FUNCTIONAL PASS" : "FUNCTIONAL FAIL");
  line(3u, "SNAPSHOT: SYNCHRONOUS HANDOFF PROXY ONLY");
  line(4u, "NORMAL / LAG / SHED / SYNTHETIC FAULT / PRESSURE");
  line(5u, "INPUT/AUDIO ARE TARGET PROXIES; NO LATENCY CLAIM");
  line(6u, "DMA: HARDWARE PROBE NOT EXECUTED");
  line(7u, timing_ok ? "RASTER ACQUISITION: ALL 80 SAMPLES VALID" : "RASTER ACQUISITION INCOMPLETE: SEE VALID MASKS");
  line(8u, "RAW MOD256 BYTES IN PHASE ORDER; WRAPS UNRESOLVED");
