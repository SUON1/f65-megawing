...
xemu_boot(){ label=$1; rm -f "$out/reports/R0E-XEMU-$label.screen.txt" "$out/reports/R0E-XEMU-$label.memory.bin" "$out/reports/R0E-XEMU-$label.png"; "$xemu" -headless -sleepless -fastboot -rom "$F65_MEGA65_ROM" -8 "$out/artifacts/F65R0E.D81" -autoload -dumpscreen "$out/reports/R0E-XEMU-$label.screen.txt" -dumpmem "$out/reports/R0E-XEMU-$label.memory.bin" -screenshot "$out/reports/R0E-XEMU-$label.png" & xemu_pid=$!; sleep "${F65_XEMU_RUN_SECONDS:-20}"; kill -TERM "$xemu_pid" 2>/dev/null || true; if wait "$xemu_pid"; then :; else :; fi; }
xemu_run(){ test -x "$xemu" || { echo 'R0-E Xemu blocked: no pinned xmega65 binary.' >&2; exit 2; }; test -n "${F65_MEGA65_ROM:-}" && test -f "$F65_MEGA65_ROM" || { echo 'R0-E Xemu blocked: set F65_MEGA65_ROM to the owner ROM outside the repository.' >&2; exit 2; }; test -f "$out/artifacts/F65R0E.D81" || { echo 'R0-E Xemu blocked: build the exact host-gated D81 first.' >&2; exit 2; }; rom_sha=$(shasum -a 256 "$F65_MEGA65_ROM" | awk '{print $1}'); test "$rom_sha" = 'af3c447f791a2fdc48cb21e1bd3fab015e32641228d9d30d21259b9e878c6fa0' || { echo "R0-E Xemu blocked: unexpected ROM SHA-256: $rom_sha" >&2; exit 2; }; mkdir -p "$out/reports"; printf '%s\n' "xmega65=$xemu" "rom_sha256=$rom_sha" "d81_sha256=$(shasum -a 256 "$out/artifacts/F65R0E.D81" | awk '{print $1}')" 'arguments=-headless -sleepless -fastboot -rom <owner ROM> -8 F65R0E.D81 -autoload' > "$out/reports/R0E-XEMU-INVOCATION.txt"; xemu_boot boot1; xemu_boot boot2; python3 "$root/tools/diagnostics/r0e_validate_xemu.py" "$root"; }
case ${1:-} in
