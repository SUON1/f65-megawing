...
build() {
  ...
  "$petcat" -w65 -o "$out/artifacts/AUTOBOOT.C65" -- "$root/src/r0c/media_boot.bas"
  "$petcat" -w65 -o "$out/artifacts/R0C-MEDIA.C65" -- "$root/src/r0c/media_fixture.bas"
  "$c1541" -format 'F65 R0-C MEDIA,65' d81 "$out/artifacts/R0CMEDIA.D81" -write "$out/artifacts/AUTOBOOT.C65" autoboot.c65 -write "$out/artifacts/R0C-MEDIA.C65" r0c-media -list > "$out/reports/R0CMEDIA.D81-create.txt" 2>&1
...
xemu_run() {
  ...
  "$xemu" -headless -sleepless -fastboot -rom "$F65_MEGA65_ROM" -9 "$out/artifacts/R0CMEDIA.D81" -prg "$out/artifacts/AUTOBOOT.C65" -prgmode 65 -dumpscreen "$out/reports/R0C-MEDIA-XEMU.screen.txt" -screenshot "$out/reports/R0C-MEDIA-XEMU.png" &
  pid=$!; sleep "${F65_XEMU_RUN_SECONDS:-20}"; kill -TERM "$pid" 2>/dev/null || true; wait "$pid" || true
