...
build() {
  ...
  "$c1541" -format 'F65 R0-C PROOF,65' d81 "$out/artifacts/R0CFINAL.D81" -write "$out/artifacts/AUTOBOOT.C65" autoboot.c65 -write "$out/artifacts/F65-R0C-PROOF.prg" r0c-final -write "$out/R0CPROOF.PKG" r0cproof -list > "$out/reports/R0CFINAL.D81-create.txt" 2>&1
...
xemu_run() {
  ...
  "$xemu" -headless -sleepless -fastboot -rom "$F65_MEGA65_ROM" -8 "$out/artifacts/R0CFINAL.D81" -autoload -dumpscreen "$out/reports/R0C-XEMU.screen.txt" -dumpmem "$out/reports/R0C-XEMU.memory.bin" -screenshot "$out/reports/R0C-XEMU.png" &
