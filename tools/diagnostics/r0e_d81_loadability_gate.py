#!/usr/bin/env python3
"""Fail-closed structural and extracted-content gate for the fresh R0-E D81."""
...
if (label,ident)!=("F65 R0-E3","65"): fail("disk label/ID mismatch")
free=set()
for t in range(1,81):
...
(out/"reports/R0E-D81-LOADABILITY.md").write_text("# R0-E D81 Loadability\n\nState: `HOST_CONTENT_VERIFIED`\n\n- Candidate: `%s`\n- SHA-256: `%s`\n- Host structural/content: `PASS`\n- Xemu: `AWAITING PUBLICATION_AND_STAGE_3`\n- Physical chooser: `AWAITING HUMAN`\n"%(image.name,sha(image)))
print("R0-E D81 STRUCTURAL+CONTENT PASS sha256="+sha(image))
