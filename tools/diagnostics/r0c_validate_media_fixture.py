...
required_fixture = [
    ...
    "trap 9000", "blank media is valid", "initialize writes only", "aa=-1:bb=-1",
    "5120 open 2,d,2,\"r0csel,s,r\"", "5130 input#2,m$,aa,bb,c$", "5140 close 2",
    "for n=1 to 512:input#2,x$,xn", "goto 40",
]
...
for forbidden in (
    ...
    if forbidden in fixture:
        raise SystemExit("R0-C media fixture validation failed: obsolete unsafe form=%s" % forbidden)
...
if not listing.exists():
    raise SystemExit("R0-C media D81 validation failed: media listing absent")
...
# The fixture program is deliberately part of this image. Its expected hash is
# build-derived and captured by the manifest; reproducibility is tested by two
# independent clean builds rather than pinning a stale carrier hash here.
...
if entries[:3] != expected_entries or len(entries) != 4:
    raise SystemExit("R0-C media D81 validation failed: physical-control layout=%s" % entries)
media_entry = entries[3]
if media_entry[:3] != ("r0c-media", 39, 11) or media_entry[3] < 1:
    raise SystemExit("R0-C media D81 validation failed: media append layout=%s" % media_entry)
print("R0-C device-9 media fixture validation PASS sha256=%s" % candidate_hash)
