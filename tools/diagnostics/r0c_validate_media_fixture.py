...
listing = root / "build/r0c/reports/R0CMEDIA.D81-list.txt"
if listing.exists():
    text = listing.read_text(errors="replace").lower()
    absent = [name for name in ("autoboot", "r0c-media") if name not in text]
    if absent:
        raise SystemExit("R0-C media D81 validation failed: absent=%s" % absent)
    forbidden = [name for name in ("r0c-final", "r0cproof") if name in text]
    if forbidden:
        raise SystemExit("R0-C media D81 validation failed: non-fixture file=%s" % forbidden)
