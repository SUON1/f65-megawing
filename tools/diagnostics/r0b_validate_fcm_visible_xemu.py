...
if len(result) != 96 or result[:8] != b"R0B2\x02\x00\x03\x04" or result[46:48] != b"\x60\x00":
    raise SystemExit(f"R0B-FCM-VIS-XEMU-001 FAIL: result header {result.hex()}")
expected_status = [3, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3]
...
if result[84] != 0x05 or result[85:89] != b"\x01\x00\x01\x01" or (sum(result[:83]) & 0xff) != result[83]:
    raise SystemExit(f"R0B-FCM-VIS-XEMU-001 FAIL: flags/checksum {result[84:89].hex()}/{result[83]:02x}")
markers = ("R0B-FCM-VIS-001 CONTEXT DEFERRED", "D031 40-PAIR PRECONDITION: FAIL", "D060 DEFAULT $0800 PRECONDITION: PASS", "FCM 64-BYTE CARD ALIGNMENT: PASS", "SCREEN BYTES EXACT RESTORE: PASS", "FCM CARD BYTES EXACT RESTORE: PASS", "OBSERVED D031/D060-D063 IN RESULT", "GATE OPEN; NO POINTER-SWAP OR PALETTE CL")
...
evidence = {
    ...
    "status": "DEFERRED",
    "scope": "Xemu runtime display-context capture. The active D031/screen-pointer configuration was not admitted, so this binary does not write D054 or claim FCM presentation.",
    ...
    "result_block": {"address": "$1800", "size": 96, "statuses": expected_status, "reasons": expected_reason, "flags": result[84], "card_alignment": result[85], "d054_restored": result[86], "screen_restored": result[87], "card_restored": result[88], "observed_registers": {"d018": result[89], "d031": result[90], "d054": result[91], "d060": result[92], "d061": result[93], "d062": result[94], "d063": result[95]}}
}
(reports / "r0b-fcm-visible-xemu-evidence.json").write_text(json.dumps(evidence, indent=2) + "\n")
print("R0B-FCM-VIS-XEMU-001 DEFERRED: runtime display context captured; no D054 write performed")
