...
def same_allocation(before, after):
    return all(before.get(key) == after.get(key) for key in (
        "D81_PATH", "D81_BYTES", "SD_FILESYSTEM", "SD_DEVICE_IDENTIFIER",
        "SD_REMOVABLE", "SD_MOUNT_POINT", "SD_EXTENT_COUNT", "SD_EXTENTS"))


def self_test():
    baseline = {"SD_EXTENTS": [{"logicalOffset": 0, "bytes": 819200, "deviceOffset": 1000}]}
    assert same_allocation(baseline, dict(baseline))
    assert not same_allocation(baseline, {"SD_EXTENTS": [{"logicalOffset": 0, "bytes": 819200, "deviceOffset": 2000}]})
...
def main():
    ...
    parser.add_argument("--same-allocation-as", type=pathlib.Path)
    parser.add_argument(
        "--fat32-root-only",
        action="store_true",
    ...
    if args.same_allocation_as:
        before = json.loads(args.same_allocation_as.read_text())
        if not same_allocation(before, report):
            fail("slot allocation or device changed after in-place fill")
