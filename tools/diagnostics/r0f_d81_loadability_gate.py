#!/usr/bin/env python3
"""Fail-closed structural and extracted-content gate for fresh R0-F1 D81."""
import hashlib, json, pathlib, subprocess, sys, tempfile

root=pathlib.Path(sys.argv[1]).resolve(); image=pathlib.Path(sys.argv[2]).resolve(); art=root/"build/r0f"; out=root/"build/r0f"
def fail(s): raise SystemExit("R0-F D81 loadability gate failed: "+s)
def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
if image.name != "F65R0F1.D81": fail("unexpected or retired carrier identity: "+image.name)
...
if (label,ident)!=("F65 R0-F1","65"): fail("disk label/ID mismatch")
free=set()
for t in range(1,81):
