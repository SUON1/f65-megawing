#!/usr/bin/env python3
"""Validate two clean R0-E Xemu functional-proxy boots without timing claims."""
import hashlib
import json
import pathlib
import subprocess
...
def validate(label):
    ...
    if len(block) != 256 or block[:8] != b"R0E1\x02\x64\x15\x7f":
    ...
    markers = ["R0-E COMBINED-LOAD FUNCTIONAL PROXY", "SNAPSHOT OWNERSHIP", "TIMING: NOT MEASURED", "DMA: HARDWARE PROBE NOT EXECUTED"]
...
evidence = {
    ...
    "class": "XEMU_FUNCTIONAL_PROXY",
    "result": "PASS",
    "sourceCommit": commit,
    "d81Sha256": sha(artifacts / "F65R0E.D81"),
    "tests": ["R0E-TICK-001", "R0E-SNAP-001", "R0E-RENDER-001", "R0E-FAULT-001", "R0E-INPUT-AUDIO-001", "R0E-STORAGE-001"],
    "dma": "DMA_HARDWARE_PROBE_NOT_EXECUTED",
    "timing": {"status": "NOT_MEASURED", "reason": "no admitted target/Xemu cycle counter; this validates functional proxy output only"},
...
print("R0E-XEMU-001 PASS: two clean functional-proxy boots verified; timing remains NOT_MEASURED")
